 ...
 <td style="vertical-align: top; width: 500px">
  ...
  <p>
   <select id="acl_what" onchange="target_change()">
    <optgroup label="<?= wfMsg('hacl_edit_protect') ?>">
    <?php foreach(explode(',', 'page,namespace,category'.(defined('SMW_NS_PROPERTY') ? ',property' : '')) as $k) { ?>
     <option id="acl_what_<?= $k ?>" value="<?= $haclgContLang->mPetPrefixes[$k] ?>"><?= wfMsg("hacl_define_$k") ?></option>
    <?php } ?>
    </option>
    <optgroup label="<?= wfMsg('hacl_edit_define') ?>">
    <?php foreach(explode(',', 'right,template') as $k) { ?>
     <option id="acl_what_<?= $k ?>" value="<?= $haclgContLang->mPetPrefixes[$k] ?>"><?= wfMsg("hacl_define_$k") ?></option>
    <?php } ?>
    </option>
   </select>
   <input type="text" autocomplete="off" id="acl_name" onchange="target_change()" onkeyup="target_change()" style="width: 200px" />
  </p>
 </td>
...
<p id="acl_pns">
 ...
 <input type="submit" name="wpSave" value="<?= wfMsg($aclSD ? 'hacl_edit_save' : 'hacl_edit_create' ) ?>" />
</p>
